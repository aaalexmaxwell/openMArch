...
void sendTweet(){
    ...
    String [] tempName = split(imLoc, File.separator);
    String imNameTag = tempName[tempName.length-2];
    String imName = imNameTag + ".jpg";
    int s = second();  
    int m = minute();  
    int h = hour();    
    ...
    try {
      save(imName);
      String message = "#simBotBETA" + " oh good...another simulation..." + " #" +teamName + " #" + imNameTag +time; // + " sent-" h + "-" + m + "-" + s;
