...
class OBJexporter
		...
		@all_mats.each{|mat| 
			...
			faces.each{|face|
				...
				# if not mat
				  ...
				  # end#if
				# else
				#send data to obj_file, with material name as group id
				  # self.export_obj_file(objname, tr, mat)
				#send data to obj_file, with material name as group id
				# end#if
				###########################################################################
	...
	def export_obj_file(objname="", tr=nil, mat=nil)
		...
		###loop through the all of the vertices of the mesh and write data to file
		if @meshes.length != 0 and @vs.length != 0
            ###
			#@obj_file.puts("g #{mat_name}")
			@obj_file.puts("o #{objname}-#{mat_name}-#{rand(1000-0)}")
			...
			@vs.each{|v|
				...
				newv="v #{"%.12g" % ((xx.to_m.to_f))} #{"%.12g" % ((zz.to_m.to_f)*-1)} #{"%.12g" % (yy.to_m.to_f)}"
			...
			if vertCount > 4 or vertCount == 3
				...
				#f_st="f #{@count_vs.length-3} #{@count_vs.length-1} #{@count_vs.length-2} #{@count_vs.length}"
				f_st="f #{@count_vs.length} #{@count_vs.length-2} #{@count_vs.length-1} #{@count_vs.length-3}"
				@obj_file.puts(f_st)
			end  
